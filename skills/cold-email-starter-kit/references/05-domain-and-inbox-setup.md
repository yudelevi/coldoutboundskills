# 05 — Domain and Inbox Setup (Dynadot + Zapmail)
...
### Recommended TLD
- `.com` — primary choice (~$10-14 on Dynadot). Highest trust, best deliverability. Default to this.
- `.co` — secondary (~$8-30 depending on registrar and name). Looks professional, often available when .com is taken.
- Avoid `.info`, `.xyz`, `.click`, `.top`, `.buzz`, `.loan` — lower inbox trust with recipients.
...
- Set a max price cap (recommended: $15 for `.com`, $12 for `.co`)
...
```
Zapmail pushes SMTP+IMAP credentials to Smartlead and creates the email accounts there. They appear in Smartlead under Email Accounts within 30 seconds. Assign them to a campaign via `POST /campaigns/{id}/email-accounts`.

### Exporting to Instantly
...
```
Same flow as Smartlead — Zapmail pushes credentials, accounts appear in Instantly under Email Accounts. Assign via the campaign create endpoint `POST /api/v2/campaigns` and include the accounts in the `email_list` field.

**First-time setup (both platforms):** You must link your sending platform account to Zapmail once. Do it in the Zapmail dashboard OR via:
```
POST https://api.zapmail.ai/api/v2/exports/accounts/third-party
Body: {"email": "you@example.com", "password": "your-platform-password", "app": "SMARTLEAD"}
```
(Use `INSTANTLY` instead of `SMARTLEAD` for Instantly.)
